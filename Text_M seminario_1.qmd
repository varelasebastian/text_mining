...
```
Una vez que tenemos los listados, podemos proceder a la eliminación. Lo haremos sobre el objeto que contiene las palabras de los títulos, por un lado, y en el objeto que tiene las de los títulos y resumenes, integradas. Esto lo hacemos para generar 2 nubes de palabras distintas que podremos comparar.
...
Con los dos pasos anteriores -el de la tokenización y el de la eliminación de palabras no significativas-, hemos terminado de preparar la versión más simple de nuestro datos. El primer análisis posible es realizar un conteo de frecuencia de palabras como para ver si esa información nos dice algo sobre los perfiles temáticos de la producción científica argentina sobre Sociología. Vamos a incorporar en este análisis tambien la variable Tópicos. Si bien provee menos cantidad de texto, como no la hemos tokenizado, mantiene la vinculación entre términos compuestos. 
...
topicos <- socio_ar_selec %>%
  unnest_longer (topics) %>%                                                                          #despliega el dataframe dentro del dataframe general
  mutate(topics_id = topics$id, topics_type = topics$type, topics_name = topics$display_name) %>%     #renombra las columnas
  filter(topics_type == "topic") %>%                                                                  #filtra por el typo "topic"
  select(topics_id,topics_name)                                                                       #selecciono lo que quiero
...
Obtención de los bigramas de los títulos ordenados por freq
...
Antes de la generación de las nubes de palabras, aplicaremos, si lo creemos necesario, algún criterio de poda por frecuencia. En este caso, generaremos 6 nubes de palabras para poder comparar variantes. Por un lado generaremos 3 nubes, la de los títulos, la de los títulos + resumen y la de tópicos, con aquellos términos que tienen frecuencia mayor a 1. Luego lo repetiremos para aquellos términos con frecuencia mayor a 10.
...
Ahora de los tópicos
```{r}
wordcloud2(bigram_tit_f, size = 0.7)
```

Ahora hagamosló para frecuencia mayor a 10

Filtrar palabras con frecuencia mayor a 10 en el títulos:
```{r}
bigram_tit_f10 <- bigram_tit %>%
  filter(n > 10) 
```

Ahora en Título + resumen
```{r}
bigram_tit_res_f10 <- bigram_tit_res %>%
  filter(n > 10) 
```
...
Creación de la nube de palabras de los títulos:
```{r}
wordcloud2(bigram_tit_f10, size = 0.7)
```
