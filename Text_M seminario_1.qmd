...
# PARTE I: Minería de textos (introductorio)

Se requieren los paquetes openalexR, tidyverse, DataExplorer, tidytext, textclean, stopwords, wordcloud2. Instalar con cuadros de diálogo y no con código.
...
Con los dos pasos anteriores -el de la tokenización y el de la eliminación de palabras no significativas-, hemos terminado de preparar la versión más simple de nuestro datos. El más simple es realizar un conteo de frecuencia de palabras como para ver si esa información nos dice algo sobre los perfiles temáticos de la producción científica argentina sobre Sociología. Vamos a incorporar en este análisis tambien la variable topics. Si bien provee menos cantidad de texto, como no la hemos tokenizado, mantiene la vinculación entre términos compuestos. 
...
topicos <- socio_ar_selec %>%
  ...
  filter(topics_type == "topic") %>%                                           select(topics_id,topics_name)
```
...
Como hemos visto hasta aquí, trabajar con "bag of word" tiene el inconveniente de la desvinculación de las palabras que componen términos compuestos. Un tipo de procesamiento que se puede aplicar para tratar de encontrar una mejora en este sentido, es con la detección de bigramas, es decir con ventanas de dos palabras. También podrían ser tri-gramas, cuatri-gramas, y así. 
...
Con un código similar, se pueden crear, por ejemplo, la de bigramas de títulos y la de tópicos
